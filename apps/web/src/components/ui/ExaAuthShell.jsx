        ...
        <div className="relative grid w-full max-w-5xl overflow-hidden rounded-[28px] border border-[var(--exa-border)] bg-[rgba(8,9,11,0.84)] shadow-[0_32px_90px_rgba(0,0,0,0.52)] backdrop-blur-xl lg:grid-cols-[0.95fr_1fr]">
          <aside className="hidden border-r border-[var(--exa-border-subtle)] bg-[linear-gradient(145deg,rgba(214,178,75,0.12),rgba(255,255,255,0.018)),var(--exa-bg-secondary)] p-8 lg:flex lg:flex-col lg:justify-between">
            <div>
              ...
              <h2 className="mt-8 max-w-sm text-4xl font-semibold leading-tight text-white">Secure global finance, built for digital assets.</h2>
              <p className="mt-4 max-w-sm text-sm leading-6 text-[var(--exa-text-muted)]">Trade, convert, transfer and manage assets from one premium ExaEarn account.</p>
            </div>
            <div className="grid gap-3 text-xs text-[var(--exa-text-muted)]">
              <div className="rounded-2xl border border-[var(--exa-border-subtle)] bg-white/[0.025] p-3">Protected access and account security controls</div>
              <div className="rounded-2xl border border-[var(--exa-border-subtle)] bg-white/[0.025] p-3">Unified wallet, trading and financial workflows</div>
            </div>
          </aside>
