      ...
      <div className="mx-auto flex w-full max-w-[1800px] flex-col gap-3 px-2 pb-[calc(env(safe-area-inset-bottom)+12px)] pt-[calc(env(safe-area-inset-top)+8px)] sm:px-3 lg:px-4">
        ...
        <div className="grid gap-3 lg:hidden">
          {mobileMode === 'chart' ? (
            ...
            <div className="grid min-w-0 grid-cols-[minmax(0,1.1fr)_minmax(112px,0.9fr)] gap-2 overflow-hidden rounded-2xl border border-white/8 bg-[#070d16] p-2">
              <OrderEntryPanel
                market={selectedMarket}
                form={form}
                onForm={setForm}
...
function ChartPanel({ loading, error, candles, timeframe, onTimeframe, chartType, onChartType, onRefresh, mobile = false }: any) {
  return <div className="rounded-2xl border border-white/8 bg-[#070d16] p-2.5"><div className="flex flex-col gap-2 border-b border-white/8 pb-2 sm:flex-row sm:items-center sm:justify-between"><div className="flex items-center gap-1 overflow-x-auto">{TIMEFRAMES.map((item) => <button key={item} type="button" onClick={() => onTimeframe(item)} className={`shrink-0 rounded-lg px-2.5 py-1.5 text-xs ${timeframe === item ? 'bg-amber-400 text-slate-950' : 'bg-white/[0.04] text-slate-300'}`}>{item.replace('h', 'H')}</button>)}</div><div className="flex items-center gap-1 overflow-x-auto"><IconToggle icon={CandlestickChart} active={chartType === 'candles'} onClick={() => onChartType('candles')} /><IconToggle icon={ChartNoAxesCombined} active={chartType === 'line'} onClick={() => onChartType('line')} /><IconToggle icon={AreaChart} active={chartType === 'area'} onClick={() => onChartType('area')} /><button type="button" onClick={onRefresh} className="inline-flex items-center gap-1 rounded-lg bg-white/[0.04] px-2.5 py-1.5 text-xs text-slate-300"><RefreshCcw className="h-3.5 w-3.5" />Refresh</button></div></div><TradingChart candles={candles} chartType={chartType} loading={loading} error={error} heightClassName={mobile ? 'h-[320px]' : 'h-[430px]'} onRetry={onRefresh} /></div>;
}
function OrderEntryPanel({ market, form, onForm, activePrice, currentPricePrecision, availableBaseBalance, availableQuoteBalance, totalDecimal, estimatedFee, feeRate, minAmount, maxAmount, validationMessage, formError, orderMessage, submitting, onApplyPercentage, onSubmit, compact = false }: any) {
  return <div className={`rounded-2xl border border-white/8 bg-[#070d16] ${compact ? 'p-2' : 'p-2.5'}`}><div className="grid grid-cols-2 gap-1 rounded-xl bg-white/[0.04] p-1"><button type="button" onClick={() => onForm((current: OrderFormState) => ({ ...current, side: 'buy' }))} className={`rounded-lg py-2 text-sm font-semibold ${form.side === 'buy' ? 'bg-emerald-500 text-white' : 'text-slate-400'}`}>Buy</button><button type="button" onClick={() => onForm((current: OrderFormState) => ({ ...current, side: 'sell' }))} className={`rounded-lg py-2 text-sm font-semibold ${form.side === 'sell' ? 'bg-rose-500 text-white' : 'text-slate-400'}`}>Sell</button></div><div className="mt-2 flex items-center justify-between text-[11px] text-slate-400"><span>Available</span><span className="font-mono">{form.side === 'buy' ? `${availableQuoteBalance.toFixed(2)} ${market?.quote || ''}` : `${availableBaseBalance.toFixed(6)} ${market?.base || ''}`}</span></div><div className="mt-2 flex gap-1 overflow-x-auto">{ORDER_TYPES.map((type) => <button key={type} type="button" onClick={() => onForm((current: OrderFormState) => ({ ...current, type, price: type === 'market' ? '' : current.price }))} className={`shrink-0 rounded-lg px-2.5 py-1.5 text-[11px] uppercase ${form.type === type ? 'bg-white text-slate-950' : 'bg-white/[0.04] text-slate-300'}`}>{type.replace('_', ' ')}</button>)}</div>{form.type !== 'market' ? <MiniField label="Price" suffix={market?.quote || 'USDT'} value={form.price} onChange={(value) => onForm((current: OrderFormState) => ({ ...current, price: value }))} placeholder={activePrice ? activePrice.toFixed(currentPricePrecision) : '0.00'} /> : null}{form.type === 'stop_loss' ? <MiniField label="Stop" suffix={market?.quote || 'USDT'} value={form.stopPrice} onChange={(value) => onForm((current: OrderFormState) => ({ ...current, stopPrice: value }))} placeholder="0.00" /> : null}<MiniField label="Quantity" suffix={market?.base || 'BTC'} value={form.amount} onChange={(value) => onForm((current: OrderFormState) => ({ ...current, amount: value }))} placeholder="0.000000" /><div className="mt-2 grid grid-cols-5 gap-1">{[0, ...PERCENTAGES].map((percent) => <button key={percent} type="button" onClick={() => percent === 0 ? onForm((current: OrderFormState) => ({ ...current, amount: '' })) : onApplyPercentage(percent)} className="rounded-lg bg-white/[0.04] py-1.5 text-[11px] text-slate-300">{percent === 0 ? '0' : `${percent}%`}</button>)}</div><div className="mt-2 rounded-xl border border-white/8 bg-white/[0.03] p-2 text-[11px] text-slate-400"><Line label="Order value" value={`${totalDecimal.gt(0) ? totalDecimal.toFixed(6) : '0.000000'} ${market?.quote || ''}`} /><Line label="Fee" value={`${estimatedFee.gt(0) ? estimatedFee.toFixed(6) : '0.000000'} ${market?.quote || ''}`} /><Line label="Fee rate" value={`${feeRate.mul(100).toFixed(2)}%`} /><Line label="Min / Max" value={`${minAmount.toString() || '0'} / ${maxAmount.gt(0) ? maxAmount.toString() : 'No cap'} ${market?.base || ''}`} /></div>{formError ? <div className="mt-2 rounded-lg border border-rose-500/20 bg-rose-500/10 px-2.5 py-2 text-[11px] text-rose-200">{formError}</div> : null}{!formError && orderMessage ? <div className="mt-2 rounded-lg border border-white/8 bg-white/[0.03] px-2.5 py-2 text-[11px] text-slate-200">{orderMessage}</div> : null}<button type="button" disabled={submitting || Boolean(validationMessage)} onClick={onSubmit} className={`mt-2 w-full rounded-xl px-3 py-2.5 text-sm font-semibold ${form.side === 'buy' ? 'bg-emerald-500 text-white' : 'bg-rose-500 text-white'} disabled:cursor-not-allowed disabled:opacity-55`}>{submitting ? 'Submitting...' : `${form.side === 'buy' ? 'Buy' : 'Sell'} ${market?.base || ''}`}</button></div>;
}
